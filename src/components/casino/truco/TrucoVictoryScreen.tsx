...
export function TrucoVictoryScreen() {
  const result = useTrucoRun((s) => s.lastResult);
  useGameOutcome(result ? (result.won ? "win" : "loss") : null);
  const reason = useTrucoRun((s) => s.lastEndReason);
  const ack = useTrucoRun((s) => s.ackResult);
