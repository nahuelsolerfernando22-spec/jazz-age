...
export function ChinchonVictoryScreen() {
  const result = useChinchonRun((s) => s.lastResult);
  useGameOutcome(result ? (result.won ? "win" : "loss") : null);
  const reason = useChinchonRun((s) => s.lastEndReason);
  const ack = useChinchonRun((s) => s.ackResult);
