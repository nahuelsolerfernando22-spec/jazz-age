...
import { BAGATELLE_LEVELS, findBagatelleLevel } from "@/lib/games/bagatelle/bagatelle-levels";
...
const REASON_LABEL: Record<string, string> = {
  ...
  "lost-balls": "Se te acabaron las bolas",
  "lost-budget": "La cantina se comió el fondo",
  "lost-clock": "El reloj te ganó",
  abandoned: "Abandonaste el encargo",
};

export function BagatelleVictoryScreen() {
  const result = useBagatelleRun((s) => s.lastResult);
  useGameOutcome(result ? (result.won ? "win" : "loss") : null);
  const reason = useBagatelleRun((s) => s.lastEndReason);
  const ack = useBagatelleRun((s) => s.ackResult);
  const startRun = useBagatelleRun((s) => s.startRun);

  return (
    <AnimatePresence>
      {result ? (
        <motion.div
          key="bagatelle-result"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          ...
          >
            ...
            <p className="mt-1 text-center text-sm text-[var(--ivory)]/75">
              {findBagatelleLevel(result.levelId)?.title}
            </p>
            ...
            {result.won ? (
              <div className="my-6 text-center">
                ...
                <p className="mt-2 font-display text-xs uppercase tracking-[0.25em] text-[var(--smoke)]">
                  {result.ballsUsed} bolas · {result.chipsGained >= 0 ? "+" : ""}
              ...
              <div className="my-6 text-center text-sm text-[var(--ivory)]/60">
                La palanca todavía funciona. Cargá otra bola cuando estés listo.
              </div>
            )}
            ...
            <div className="flex gap-2">
              ...
              {result.won ? (
                (() => {
                  const cur = BAGATELLE_LEVELS.find((l) => l.id === result.levelId);
                  const next = cur ? BAGATELLE_LEVELS[cur.order] : undefined;
                  if (!next) return null;
                  return (
                    <button
                      type="button"
